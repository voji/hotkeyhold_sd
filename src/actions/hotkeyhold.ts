 ...
 */
@action({ UUID: "hu.voji.keyboard.hotkeyhold" })
export class HotkeyHold extends SingletonAction<HotkeyHoldSettings> {
	 ...
	 */
	override async onKeyDown(ev: KeyDownEvent<HotkeyHoldSettings>): Promise<void> {
	 ...
	 */
	override async onKeyUp(ev: KeyUpEvent<HotkeyHoldSettings>): Promise<void> {
		const { settings } = ev.payload;
		var shift = typeof settings.mod_shift === 'boolean' ? settings.mod_shift : false;
		var ctrl = typeof settings.mod_ctrl === 'boolean' ? settings.mod_ctrl : false;
		var alt = typeof settings.mod_alt === 'boolean' ? settings.mod_alt : false;
	 ...
	 */
	override async onWillAppear(ev: WillAppearEvent<HotkeyHoldSettings>): Promise<void> {
		const { settings } = ev.payload;
	...
	*/
	override async onDidReceiveSettings(ev: DidReceiveSettingsEvent<HotkeyHoldSettings>): Promise<void> {
		const { settings } = ev.payload;
		await this.updateTitle(ev, settings);
	}
	 ...
	 */
	private async updateTitle(ev: any, settings: HotkeyHoldSettings): Promise<void> {
	 ...
	 * Generates a display string from the current settings
	  */
	private generateTitleFromSettings(settings: HotkeyHoldSettings): string {
		const modifiers = [];
...
/**
 * Settings for {@link HotkeyHold}.
 */
type HotkeyHoldSettings = {
	hotkey: string;
	mod_shift: boolean;
	mod_ctrl: boolean;
	mod_alt: boolean;
